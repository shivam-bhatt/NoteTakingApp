 ...
 obj.createNote =function()
{
           ...
           var div = document.createElement('li');
            var id = 'note-'+(count++);
             div.innerHTML = "<div class='create-note' id = '"+id+"'><img id='blah' src='#' alt='your image' /><textarea class='note-title' placeholder='Untitled' maxlength='10'></textarea><textarea class='note-content' placeholder='Your content here'></textarea><img class='delete-note' src='images.jpg' height='20' width='20'></img><input type = 'submit' value = 'save'></input><input type = 'file' class = 'upload-file' onchange='obj.readURL(this);'></div>";
...
obj.displayTrash = function ()
{
    ...
    if(trashNotesArray.length!==undefined)
        {
    for(var j=0; j<trashNotesArray.length; j++) 
    {
    var div = document.createElement('li');    
        var id = trashNotesArray[j].id;
        var title = trashNotesArray[j].title;
        var body = trashNotesArray[j].body;

       div.innerHTML += "<div class='trash-note' id = '"+id+"'><textarea class='note-title' placeholder='Untitled' maxlength='10'>"+title+"</textarea><textarea class='note-content' placeholder='Your content here'>"+body+"</textarea><img class='delete-note' src='images.jpg' height='20' width='20'><input type = 'submit' value = 'restore'><br><I>"+trashNotesArray[j].datetime+"</br></I></input></img></div>";
       
        var restoreButton = div.getElementsByTagName('input')[0];
    
            restoreButton.onclick = function() { saveNote(id); };   

     var removeNote = div.getElementsByClassName('delete-note')[0];           
            removeNote.onclick = function() { deleteTrashNote(id);};
        
        trashNotes.appendChild(div);
    }
        }
}
 ...
 function dublicay(id)
{
 ...
 function saveNote(id)
       ...
       alert(oldNote.parentElement.getAttribute('id'));     
    if(oldNote.parentElement.getAttribute('id') == 'notes')
                {
    ...
    else 
        {
            ...
            saveButton.onclick = function() {saveNote(id);};
             var removeNote = div.getElementsByClassName('delete-note')[0];           
            removeNote.onclick = function() { obj.deleteNote(id);};
